...
class MVDadHats(Brands):
    ...
    def parse_to_dictionary(self, content):
        product_info = {
            ...
            "Retail Price": round(content["Sale Price"] * 3.3325, 2),  # retail price / cost = 3.3325x
            "Sizes and Quantities": [{"O/S": content["QTY"]}]
            # size (string) : quantity (integer), hats are one size (O/S)
        }
